...
LogicalSessionRecord makeRecord(Date_t time = Date_t::now()) {
    return LogicalSessionRecord::makeAuthoritativeRecord(SignedLogicalSessionId::gen(), time);
}
...
public:
    void run() {
        ...
        resRefresh = collection()->refreshSessions(opCtx(), {record2.getId().getLsid()}, now);
        ASSERT(resRefresh.isOK());

        swRecord = collection()->fetchRecord(opCtx(), record2.getId());
        // TODO post-rebase with Jason's code, reenable this check.
        // ASSERT(swRecord.isOK());
