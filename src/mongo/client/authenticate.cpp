...
void authMongoCR(RunCommandHook runCommand, BSONObj params, AuthCompletionHandler handler) {
...
void auth(RunCommandHook runCommand,
    ...
    if (mechanism == kMechanismMongoCR)
        return authMongoCR(runCommand, std::move(params), handler);

#ifdef MONGO_CONFIG_SSL
    else if (mechanism == kMechanismMongoX509)
        return authX509(runCommand, std::move(params), clientName, handler);
#endif

    else if (saslClientAuthenticate != nullptr)
        return saslClientAuthenticate(runCommand, hostname, std::move(params), handler);
...
void authenticateClient(BSONObj params,
                        StringData hostname,
                        StringData clientName,
