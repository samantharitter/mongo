...
void asyncSaslConversation(RunCommandHook runCommand,
                           std::shared_ptr<SaslClientSession> session,
                           BSONObj saslCommandPrefix,
                           BSONObj inputObj,
                           StringData targetDatabase,
                           int saslLogLevel,
    ...
    if (session->isDone()) {
        bool isServerDone = inputObj[saslCommandDoneFieldName].trueValue();
        if (isServerDone) {
            return handler(
                AuthResponse(ErrorCodes::ProtocolError, "Client finished before server."));
        }
        return handler(AuthResponse(Status::OK()));
    }
    ...
    runCommand(request,
               [runCommand, session, targetDatabase, saslLogLevel, handler](AuthResponse response) {

                   auto code = ErrorCodes::fromInt(
                       response.getValue().data[saslCommandCodeFieldName].numberInt());

                   // Server versions 2.3.2 and earlier may return "ok: 1" with a non-zero
                   // "code" field, iindicating a failure.  Subsequent versions should
                   ...
                   // indicate failure.
                   if (!response.isOK() || code != ErrorCodes::OK) {
                   ...
                   asyncSaslConversation(runCommand,
                                         ...
                                         response.getValue().data.getOwned(),
                                         targetDatabase,
                                         saslLogLevel,
                                         handler);
               });
 ...
 * "client".
 */
void saslClientAuthenticateImpl(RunCommandHook runCommand,
                                StringData hostname,
                                const BSONObj& saslParameters,
                                AuthCompletionHandler handler) {
