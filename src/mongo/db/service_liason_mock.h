...
public:
    ...
    void clear();
    void fastForward(Milliseconds interval);

private:
    stdx::mutex _mutex;
    SessionList _activeSessions;

    AtomicWord<Milliseconds> _fastForward;
};
...
public:
    ...
    MockServiceLiason(std::shared_ptr<MockServiceLiasonImpl> impl) : _impl(std::move(impl)) {}
