...
public:
    ...
    void help(std::stringstream& help) const override {
        help << "end a list of logical sessions";
    }
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        if (AuthorizationManager::get(serviceContext)->isAuthEnabled()) {
            ...
            auto res = authzSession->getSingleUser();
            if (!res.isOK()) {
                return appendCommandStatus(
                                           result,
                                           {ErrorCodes::Unauthorized,
                                                   "must be authenticated as exactly one user "
                                                   "to call endSessions"});
            }
        ...
        auto elem = cmdObj["endSessions"];
        if (elem.type() != BSONType::Array) {
            return appendCommandStatus(
                                       result,
                                       {ErrorCodes::TypeMismatch, "endSessions must be of type array"});
        }

        // TODO SERVER-28336: implement command
