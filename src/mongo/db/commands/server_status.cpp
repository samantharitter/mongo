...
public:
    ...
    BSONObj generateSection(OperationContext* txn, const BSONElement& configElement) const {
        BSONObjBuilder bb;
        auto tl = txn->getServiceContext()->getTransportLayer();
        bb.append("current", tl->numOpenSessions());
        bb.append("available", tl->numAvailableSessions());
        bb.append("totalCreated", tl->numCreatedSessions());
        return bb.obj();
    }
