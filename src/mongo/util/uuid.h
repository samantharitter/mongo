...
public:
    ...
    /**
     * Construct a UUID from the given BSON element. Caller must validate the input.
     */
    UUID(BSONElement from);

    /**
     * Construct a UUID from the given string.
     */
    ...
    struct Hash {
        uint32_t operator()(const UUID& uuid) const {
            uint32_t hash;
            MurmurHash3_x86_32(&(uuid._uuid[0]), 16, 0, &hash);
            return hash;
        }
    };
