...
TEST(UUIDTest, UUIDCollisionTest) {
    ...
    std::unordered_set<UUID, UUID::Hash> uuids;
...
TEST(UUIDTest, toAndFromString) {
    ...
    auto uuid3 = UUID(s3);
    auto uuid3Again = UUID(s3);
    ASSERT_EQUALS(uuid3, uuid3Again);
    ...
    auto uuidLower = UUID(sLower);
    auto uuidUpper = UUID(sUpper);
    ...
    ASSERT_NOT_EQUALS(UUID(s4), UUID(s5));
}
