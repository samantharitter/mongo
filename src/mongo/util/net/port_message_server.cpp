...
namespace {
    ...
    private:
         ...
         */
    PortMessageServer::PortMessageServer(  const MessageServer::Options& opts, MessageHandler * handler ) :
        Listener( "" , opts.ipList, opts.port ), _handler(handler) {
    }

    void PortMessageServer::accepted(boost::shared_ptr<Socket> psocket, long long connectionId ) {
    ...
    void PortMessageServer::setAsTimeTracker() {
        Listener::setAsTimeTracker();
    }

    void PortMessageServer::setupSockets() {
        Listener::setupSockets();
    }

    void PortMessageServer::run() {
        initAndListen();
    }

    //bool PortMessageServer::useUnixSockets() const { return true; }
