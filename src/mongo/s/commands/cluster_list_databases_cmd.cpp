...
public:
    ...
    virtual bool run(OperationContext* opCtx,
        ...
        shardIds.emplace_back(kConfigServerShard);
        ...
        for (const ShardId& shardId : shardIds) {
            ...
            while (j.more()) {
                ...
                if (name == "admin" && shardId != kConfigServerShard) {
                    continue;
                }

                // We don't collection config server info for dbs other than "admin" and "config".
                if (shardId == kConfigServerShard && name != "config" && name != "admin") {
                    continue;
                }
