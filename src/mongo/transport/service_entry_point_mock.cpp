...
void ServiceEntryPointMock::startSession(SessionHandle session) {
    _threads.emplace_back(&ServiceEntryPointMock::run, this, session);
}

void ServiceEntryPointMock::run(SessionHandle session) {
    Message inMessage;
    while (true) {
        {
            stdx::lock_guard<stdx::mutex> lk(_shutdownLock);
