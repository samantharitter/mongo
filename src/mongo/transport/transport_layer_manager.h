 ...
 * underneath.
 */
class TransportLayerManager : public TransportLayer {
...
public:
    ...
    void addTransportLayer(std::unique_ptr<TransportLayer> tl);
...
private:
    ...
    template <typename Callable>
    int _sum(Callable&& cb);

    /**
     * A class for Tickets issued from the TransportLayerManager. These will
     * route to the appropriate TransportLayer when run with wait() or
     * asyncWait().
     */
    class ManagerTicket : public TicketImpl {
        MONGO_DISALLOW_COPYING(ManagerTicket);

    public:
        ManagerTicket(const Session& session, Ticket ticket);

        SessionId sessionId() const override;
        Date_t expiration() const override;

        Ticket _ticket;
        TransportLayer* _sessionTL;
    };

    stdx::mutex _tlsMutex;
    std::unordered_set<std::unique_ptr<TransportLayer>> _tls;
};
