...
Ticket TransportLayerLegacy::sourceMessage(const SessionHandle& session,
    ...
    auto legacySession = std::dynamic_pointer_cast<LegacySession>(session);
    return Ticket(this,
                  stdx::make_unique<LegacyTicket>(legacySession, expiration, std::move(sourceCb)));
}

SSLPeerInfo TransportLayerLegacy::getX509PeerInfo(const ConstSessionHandle& session) const {
    auto legacySession = std::dynamic_pointer_cast<const LegacySession>(session);
...
Ticket TransportLayerLegacy::sinkMessage(const SessionHandle& session,
    ...
    auto legacySession = std::dynamic_pointer_cast<LegacySession>(session);
    return Ticket(this,
                  stdx::make_unique<LegacyTicket>(legacySession, expiration, std::move(sinkCb)));
}
...
Status TransportLayerLegacy::_runTicket(Ticket ticket) {
    ...
    try {
        res = legacyTicket->_fill(conn->amp.get());
    } catch (...) {
...
void TransportLayerLegacy::_handleNewConnection(std::unique_ptr<AbstractMessagingPort> amp) {
    ...
    // Add to our list
    auto it = _sessions.emplace(_sessions.begin(), session);

    // Save the iterator for fast lookup later
    session->setIter(std::move(it));
