...
const auto kDefaultEnd = [](const Session& session) { return; };                     // TODO
const auto kDefaultAsyncWait = [](Ticket, TicketCallback cb) { cb(Status::OK()); };  // TODO
...
void ServiceEntryPointTestSuite::noLifeCycleTest() {
    ...
    _tl->_end = [&testComplete](const Session&) { testComplete.set_value(true); };

    // Kick off the SEP
    Session s(HostAndPort(), HostAndPort(), _tl.get());
    _sep->startSession(std::move(s));

    ASSERT(testFuture.get());
}
...
void ServiceEntryPointTestSuite::fullLifeCycleTest() {
    ...
    _tl->_sinkMessage = stdx::bind(
        &ServiceEntryPointTestSuite::MockTLHarness::_sinkOnceThenError, _tl.get(), _1, _2, _3);

    // Step 7: SEP destroys the session, which calls end()
    _tl->_end = [&testComplete](const Session& session) { testComplete.set_value(true); };
...
void ServiceEntryPointTestSuite::interruptingSessionTest() {
    ...
    // Start Session B
    _tl->_wait = [this, idA, &startB, &cv, &cvLock](Ticket t) {
        ...
        startB.set_value(true);

        stdx::unique_lock<stdx::mutex> lk(cvLock);
        cv.wait(lk);
    ...
    // Step 6: SEP calls wait() for tB, gets Status::OK()
    // Step 7: SEP calls sourceMessage() for B, gets tB
    // Step 8: SEP calls wait() for B, gets an error
    // Step 9: SEP calls end(B)
    _tl->_end = [this, idA, idB, &cv, &testComplete](const Session& session) {
        ...
        if (session.id() == idB) {
            ...
            testComplete.set_value(true);
        }
    };

    // Resume Session A
    // Step 10: SEP calls sinkMessage() for A
    // Step 11: SEP calls wait() for A, gets Status::OK()
    // Step 12: SEP calls sourceMessage() for A
    // Step 13: SEP calls wait() for A, receives an error
...
void ServiceEntryPointTestSuite::burstStressTest(int numSessions,
    ...
    _tl->_sourceMessage = stdx::bind(
        &ServiceEntryPointTestSuite::MockTLHarness::_defaultSource, _tl.get(), _1, _2, _3);
    _tl->_sinkMessage =
        stdx::bind(&ServiceEntryPointTestSuite::MockTLHarness::_defaultSink, _tl.get(), _1, _2, _3);
    ...
    _tl->_end = [&allSessionsComplete, numSessions, &ended](const Session& session) {
        if (ended.fetchAndAdd(1) == (numSessions - 1)) {
            allSessionsComplete.set_value(true);
        }
    };
