...
/**
 * Test class. Uses a mock TransportLayer to test that the ServiceEntryPoint calls
 * the expected methods on the TransportLayer in the expected order, and with the
 * expected parameters.
 *
 * Usage:
 *
 ...
 *    auto sep = stdx::make_unique<ServiceEntryPointImpl>();
 *    setServiceEntryPoint(std::move(sep));
 *
 *    // Run some tests
 *    fullLifeCycleTest();
 * }
...
public:
    // Need a function that takes a TransportLayer* and returns a new ServiceEntryPoint
...
private:
    ...
    public:
        ...
        transport::Ticket _sinkOnceThenError(const transport::Session& s,
                                             const Message& m,
                                             Date_t d);
